...
//USB中断配置
void USB_Interrupts_Config(u16 m_irq, u16 s_irq)
{ 
...
//	}
	...
	for(j=0;j<i;j++)//循环发送数据
	{
		USB_USART_SendData(fmt[j]); 
	}
} 


volatile uint32_t  usb_rcv_len = 0;

#if 1
	/*
	*  @brief ??USART2????????
	*/
	uint32_t USB_GetRcvNum(void)
	{

		usb_rcv_len = USB_USART_RX_STA;
		return usb_rcv_len;
	}

	/*
	*  @brief ??USART2???????buf,???rcv_len
	*/
	void  USB_GetRcvData(char *buf, uint32_t rcv_len)
	{
		if(buf)
		{
			memcpy(buf, USB_USART_RX_BUF, rcv_len);
		}
